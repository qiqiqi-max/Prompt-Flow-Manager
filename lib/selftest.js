...
function attachSelfTest(targetWin) {
  ...
  const consoleErrors = [];
  // 两套签名都要认。Electron 36 把这个事件改成只传一个 details 对象
  // （{ level: 'warning'|'error'|…, message, … }），旧签名是 (e, level, message)
  // 且 level 是整数（2=warning、3=error）。
  //
  // 为什么不能只写新的那套：这个监听器是"渲染进程报错就让自检失败"的唯一入口
  // （见下面 consoleErrors.length 那处 fail）。签名变了而代码没跟上时，
  // level 会变成那个 details 对象，`level >= 2` 恒为 false——于是页面里报什么错
  // 都收集不到，自检照旧全绿。这类失效不会有任何报错，只会让一道检查静默消失，
  // 所以两套都认，而不是赌运行时是哪个版本。
  targetWin.webContents.on('console-message', (...args) => {
    const d = args[0];
