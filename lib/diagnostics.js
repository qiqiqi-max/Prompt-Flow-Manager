...
// ---------- 沙箱状态的脱敏 ----------
// 入参是主进程按 lib/sandbox-state.js 的判定结果拼出来的对象。这里做两件事：
// 把 reason 卡在已知枚举里、把其余字段收成固定形状。
//
// 为什么要卡枚举而不是原样输出：reason 未来很可能被人顺手改成拼接的
// 「'renderer-gone: ' + e.message」，而那个 message 里可以带路径和文件名。
// 诊断包是要贴出去给别人看的，白名单是这里唯一能挡住那次改动的地方。
const SANDBOX_REASONS = new Set([
  'default', 'default-on', 'env-forced-on', 'env-forced-off',
  'renderer-unconfirmed', 'retry-after-unconfirmed', 'retry-after-degrade',
  'degraded', 'renderer-gone', 'load-failed'
]);
// 就地降级的理由是**带细节的**：'render-process-gone:crashed'、
// 'did-fail-load:-6:ERR_FILE_NOT_FOUND'。只查上面那个整串白名单的话，
// 每一条真实降级理由都会被压成 'other' —— 也就是最需要区分的那一刻反而没信息
// （"这台机器为什么关了沙箱"是诊断包里这一段存在的全部意义）。
//
// 所以按 '前缀:细节' 拆开：前缀必须在下面这个集合里，细节再各自收紧。
// 白名单挡路径泄漏的作用一点没松——细节部分只允许 Electron 自己的枚举值和
// Chromium 的错误码名（全大写下划线），路径里必然有的小写字母、斜杠、点、
// 盘符冒号都过不了。
const SANDBOX_REASON_PREFIXES = new Set(['render-process-gone', 'did-fail-load']);
// Electron render-process-gone 的 details.reason，是个封闭枚举。
const GONE_REASONS = new Set([
  'clean-exit', 'abnormal-exit', 'killed', 'crashed', 'oom',
  'launch-failed', 'integrity-failure', 'unknown'
]);
function sanitizeReason(raw) {
  if (typeof raw !== 'string' || !raw) return 'other';
  if (SANDBOX_REASONS.has(raw)) return raw;
  const at = raw.indexOf(':');
  if (at <= 0) return 'other';
  const head = raw.slice(0, at);
  if (!SANDBOX_REASON_PREFIXES.has(head)) return 'other';
  const detail = raw.slice(at + 1);
  if (head === 'render-process-gone') {
    return head + ':' + (GONE_REASONS.has(detail) ? detail : 'other');
  }
  // did-fail-load:<code>:<desc>，code 是负整数，desc 是 ERR_XXX 这样的常量名。
  const m = /^(-?\d{1,6}):([A-Z][A-Z0-9_]{0,63})$/.exec(detail);
  if (m) return head + ':' + m[1] + ':' + m[2];
  const only = /^(-?\d{1,6})$/.exec(detail);
  return head + ':' + (only ? only[1] : 'other');
}
function sanitizeSandbox(s) {
  if (!s || typeof s !== 'object') return null;
  return {
    enabled: s.enabled === undefined ? null : !!s.enabled,
    reason: sanitizeReason(s.reason),
    // 本次启动是否发生过就地降级。单看 enabled=false 分不出"一开始就关着"
    // 和"开着但起不来临时降的"，而这两种情况该做的事完全不同。
    degradedThisRun: s.degradedThisRun === undefined ? null : !!s.degradedThisRun,
    strikes: Number.isFinite(s.strikes) ? Math.floor(s.strikes) : 0,
    degradedAt: typeof s.degradedAt === 'string' ? s.degradedAt : null,
    lastConfirmedAt: typeof s.lastConfirmedAt === 'string' ? s.lastConfirmedAt : null,
    // 状态文件写不进去的话，降级结论跨不过重启，值得报出来
    stateWritable: s.stateWritable === undefined ? null : !!s.stateWritable
  };
}
...
//   sandbox      { enabled, reason, strikes, degradedAt, lastConfirmedAt }，
//                由主进程从 lib/sandbox-state.js 的判定结果拼出来
function collect(ctx) {
  ...
  return {
    ...
    // 沙箱状态。这是诊断包里唯一能回答"为什么我这台机器没开沙箱"的地方——
    // 降级是自动发生的、跨启动的，用户界面上看不到任何痕迹，
    // 不报出来的话一台被永久按在 no-sandbox 上的机器谁也发现不了。
    // 字段全是枚举值和时间戳，没有路径也没有文件名。
    sandbox: sanitizeSandbox(c.sandbox),
...
module.exports = {
  collect, exportTo, defaultFileName, countTree, diskFree, sanitizeHeal, sanitizeSandbox, TAIL_LINES
};
